...
router.get('/', async (req, res) => {
  try {
    ...
    if (isHR) { /* see all */ }
      ...
      const { data: directs } = await supabase.from('employees').select('id').eq('supervisor_id', req.user.employee_id);
...
router.get('/balance', async (req, res) => {
  try {
    const { data } = await supabase.from('leave_balances')
      .select('*').eq('employee_id', req.user.employee_id).single();
...
router.post('/', async (req, res) => {
  ...
  try {
    const { data, error } = await supabase.from('leave_requests')
      .insert({ employee_id: req.user.employee_id, leave_type, start_date, end_date, reason, status: 'pending' })
      .select().single();
    if (error) throw error;
