...
router.get('/', authorize('hr', 'admin', 'hr_manager', 'payroll_officer'), async (req, res) => {
  try {
    const { data, error } = await supabase.from('payroll_runs')
      .select('*').order('period_end', { ascending: false }).limit(24);
...
router.post('/run', authorize('hr', 'admin', 'hr_manager', 'payroll_officer'), async (req, res) => {
  ...
  try {
    const { data: run, error: runErr } = await supabase.from('payroll_runs')
      .insert({ period_start, period_end, pay_date, status: 'processing', created_by: req.user.id })
      .select().single();
    if (runErr) throw runErr;

    const { data: employees } = await supabase.from('employees')
      .select('id, basic_pay, allowances').eq('employment_status', 'active');
    ...
    const records = (employees || []).map(emp => {
      ...
      return {
        ...
        basic_pay: emp.basic_pay, gross_pay: result.grossPay,
        sss_ee: result.contributions.sss.employee, sss_er: result.contributions.sss.employer,
        philhealth_ee: result.contributions.philhealth.employee, philhealth_er: result.contributions.philhealth.employer,
        pagibig_ee: result.contributions.pagibig.employee, pagibig_er: result.contributions.pagibig.employer,
